// FILE: MarketingAutomationLiveFeed.tsx
'use client';
...
export default function MarketingAutomationLiveFeed() {
  ...
  return (
    <div className="w-[600px] h-[600px] overflow-hidden relative bg-[#0a0c10] text-white font-sans flex flex-col selection:bg-[#137fec]/30">
      ...
      <main className="flex-1 p-4 flex flex-col gap-3 overflow-hidden relative z-10">
        ...
        {/* Pipeline Stages - Compact Grid */}
        <div className="grid grid-cols-4 gap-2 h-16 shrink-0">
          <StageCard number="01" title="Ingest" icon={<HardDriveDownload size={14} />} colorClass="text-slate-400" bgClass="bg-white/5" />
          <StageCard number="02" title="Enrich" icon={<Database size={14} />} colorClass="text-[#137fec]" bgClass="border-[#137fec]/30 bg-[#137fec]/5" />
          <StageCard number="03" title="Score" icon={<LineChart size={14} />} colorClass="text-violet-400" bgClass="bg-white/5" />
          <StageCard number="04" title="Route" icon={<Rocket size={14} />} colorClass="text-emerald-400" bgClass="bg-white/5" />
        </div>
        ...
        <div className={`relative p-3 ${glassClass} rounded-xl overflow-hidden flex flex-col shrink-0 h-[170px]`}>
          ...
          <div className="mt-auto grid grid-cols-3 gap-2 pt-3 border-t border-white/5">
            <MiniMetric label="In Queue" value={stats.inQueue} color="text-white" />
            <MiniMetric label="Enriched" value={stats.enriched} color="text-[#137fec]" />
            <MiniMetric label="Routed" value={stats.routed} color="text-emerald-400" />
          </div>
        </div>
        ...
        <div className={`flex-1 ${glassClass} rounded-xl overflow-hidden flex flex-col min-h-0`}>
          ...
          <div className="overflow-y-auto custom-scrollbar p-1">
             <div className="flex flex-col gap-1">
                <LogItem time="14:22:01" msg={<span>Lead <span className="text-[#137fec]">#AF-2091</span> routed to Salesforce</span>} tag="ROUTED" tagColor="text-emerald-400 border-emerald-500/30" />
                <LogItem time="14:21:58" msg={<span>Lead <span className="text-[#137fec]">#AF-2092</span> scored <span className="text-violet-400 font-bold">88</span></span>} tag="SCORED" tagColor="text-violet-400 border-violet-500/30" />
                <LogItem time="14:21:55" msg={<span>Enriching <span className="text-[#137fec]">techcorp.io</span> via Clearbit</span>} tag="ENRICH" tagColor="text-[#137fec] border-[#137fec]/30" />
                <LogItem time="14:21:52" msg={<span>Ingested raw payload from Webhook</span>} tag="INGEST" tagColor="text-slate-400 border-slate-500/30" />
             </div>
          </div>
        </div>
...
function StageCard({ number, title, icon, colorClass, bgClass }: any) {
  return (
    <div className={`flex flex-col justify-center items-center p-2 rounded-lg border border-white/5 ${bgClass} relative overflow-hidden group`}>
       <div className={`absolute top-1 right-2 text-xs font-black opacity-20 italic ${colorClass}`}>{number}</div>

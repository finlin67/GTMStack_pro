  ...
  return (
    <div className="relative w-full h-full aspect-square rounded-2xl overflow-hidden group">
      ...
      <motion.div 
        ...
        className="absolute top-[25%] left-[20%] size-12 rounded-full border border-gold-400/50 bg-gold-400/5 flex items-center justify-center z-30 hover:bg-gold-400/20 transition-colors cursor-pointer"
        onClick={() => handleNodeClick('mail')}
        onMouseEnter={() => setHoveredNode('mail')}
        onMouseLeave={() => setHoveredNode(null)}
      >
        <Mail className="text-gold-400 w-5 h-5" />
        <div className="absolute -top-1 -right-1 size-3 bg-gold-400 rounded-full animate-pulse shadow-[0_0_10px_rgba(251,191,36,0.5)]" />
        <AnimatePresence>
            {hoveredNode === 'mail' && (
                <motion.div
                    ...
                    exit={{ opacity: 0, y: 5 }}
                    className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-900/90 text-gold-400 border border-gold-400/20 text-[10px] font-bold px-3 py-1.5 rounded-lg backdrop-blur-md whitespace-nowrap shadow-xl z-50 pointer-events-none"
                >
                    Mail Node
                </motion.div>
            )}
        ...
        <div className="flex-1 p-6 flex flex-col gap-4 z-20 overflow-hidden">
          ...
          <div className="flex-1 glass-card rounded-2xl p-5 border border-white/10 flex flex-col shadow-lg relative overflow-hidden">
            ...
            <div className="flex-1 flex flex-col justify-around relative z-10">
               <CompactTouchpoint icon={Mail} label="Email Sequence" value="248 Active" color="text-gold-400" barColor="bg-gold-400" />

...
const createFlapLoopVariants = (duration: number): Variants => {
  const times = [
    0,
    TIMING.flapOpenStart,
    TIMING.flapOpenEnd,
    TIMING.flapCloseStart,
    TIMING.flapCloseEnd,
    1,
  ]
...
const createEnvelopeInnerLoopVariants = (duration: number): Variants => {
  const times = [
    0,
    TIMING.flapOpenStart,
    TIMING.flapOpenEnd,
...
const createFlapInnerLoopVariants = (duration: number): Variants => {
  const times = [
    0,
    TIMING.flapOpenStart,
    TIMING.flapOpenEnd,
...
const createSealLoopVariants = (duration: number): Variants => {
  const times = [
    0,
    TIMING.flapOpenStart,
    TIMING.flapOpenEnd,
...
const createPlaneLoopVariants = (duration: number): Variants => {
  const flightX = [0, 30, 120, 180, 200, 160, 80, 20, 0]
  const flightY = [0, -40, -120, -160, -140, -80, -20, 10, 0]
  const flightScale = [0.3, 0.8, 1, 1, 1, 0.9, 0.7, 0.5, 0.3]
  const flightOpacity = [0, 1, 1, 1, 1, 1, 1, 0.8, 0]
  const flightRotate = [0, -20, -35, -15, 10, 30, 45, 60, 90]

  const flightTimesLocal = [0, 0.1, 0.3, 0.45, 0.55, 0.7, 0.85, 0.95, 1]
  const flightSpan = TIMING.planeEnterEnd - TIMING.planeEmergeStart
  const flightTimesGlobal = flightTimesLocal.map(
    (t) => TIMING.planeEmergeStart + t * flightSpan
  )

  const x = [0, ...flightX, 0]
  const y = [0, ...flightY, 0]
  const scale = [0.3, ...flightScale, 0.3]
  const opacity = [0, ...flightOpacity, 0]
  const rotate = [0, ...flightRotate, 0]

  const times = [0, ...flightTimesGlobal, 1]
...
const EmailMarketingHero: React.FC<EmailMarketingHeroProps> = ({
  ...
  const envelopeInnerVariants = useMemo(() => createEnvelopeInnerLoopVariants(duration), [duration])
  const flapLoopVariants = useMemo(() => createFlapLoopVariants(duration), [duration])
  const flapInnerVariants = useMemo(() => createFlapInnerLoopVariants(duration), [duration])
  const sealVariants = useMemo(() => createSealLoopVariants(duration), [duration])
  const planeVariants = useMemo(() => createPlaneLoopVariants(duration), [duration])

  const loopKey: keyof Variants = isPaused ? 'paused' : 'animate'
      ...
      >
        ...
        <g filter="url(#emh-shadow)">
          ...
          <motion.polygon
            ...
            variants={envelopeInnerVariants}
            initial="paused"
            animate={loopKey}
          />
          ...
          transition={
            isPaused
              ...
              : {
                  ...
                  repeat: Infinity,
                  repeatType: 'loop',
                  times: [0, TIMING.planeEmergeStart, TIMING.planeEmergeStart + 0.08, TIMING.planeEnterEnd, 1],
                  ease: 'easeInOut',
                }
          }
        >

...
export default function ProjectDetailPage({ params }: Props) {
  ...
  const industry = industryItems.find((i) => i.slug === project.industry)
  const relatedExpertise = expertiseItems
    .filter((e) => project.expertise.includes(e.slug))
    .slice(0, 3)
  const relatedProjects = caseStudyItems
    .filter((c) => c.slug !== project.slug && c.industry === project.industry)
    .slice(0, 2)

  // Extract outcome headline from title (already outcome-first)
  const outcomeHeadline = project.title
  const outcomeSubhead = project.description

  // Parse solution into route map steps (split by periods/sentences, max 5)
  const solutionSteps = project.solution
    .split(/[.!?]\s+/)
  ...
  const executionStack = [
    { category: 'Channels', items: project.tags.slice(0, 3) },
